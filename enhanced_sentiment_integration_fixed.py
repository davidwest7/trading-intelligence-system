#!/usr/bin/env python3
"""
Enhanced Sentiment Integration System - FIXED VERSION
...
class EnhancedSentimentIntegration:
    ...
    async def get_news_sentiment(self, symbol: str) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if self.news_api_key and self._check_rate_limit('news_api'):
                ...
                if data and 'articles' in data:
                    ...
                    if articles:
                        ...
                        logger.warning("⚠️ NewsAPI: No valid articles found")
                else:
                    logger.warning("⚠️ NewsAPI: No data received")
            
            # 2. RSS Feeds (simplified for now)
            rss_articles = []
            rss_feeds = [
            ...
            for feed_url in rss_feeds:
                try:
                    ...
                    for entry in feed.entries[:5]:  # Limit to 5 entries per feed
                        ...
                        if symbol.lower() in content.lower() and content.strip():
            ...
            if rss_articles:
                ...
                logger.info(f"✅ RSS Feeds: Found {len(rss_articles)} articles")
        ...
        if all_articles:
            ...
            logger.info(f"✅ News sentiment: {len(all_articles)} total articles")
        else:
    ...
    async def get_social_sentiment(self, symbol: str) -> Dict[str, Any]:
        ...
        # For now, create mock social data since we don't have all API keys
        mock_posts = [
            {
                'id': '1',
        ...
        if mock_posts:
            ...
            social_data['aggregated_sentiment'] = self._aggregate_sentiment(mock_posts)
            logger.info(f"✅ Social sentiment: {len(mock_posts)} posts")
    ...
    async def get_youtube_sentiment(self, symbol: str) -> Dict[str, Any]:
        ...
        # For now, create mock YouTube data
        mock_videos = [
            {
                'id': 'video1',
                'title': f'{symbol} Stock Analysis - Bullish Outlook',
                'description': f'Comprehensive analysis of {symbol} stock with positive outlook',
                'channelTitle': 'StockAnalysis',
                'sentiment': self._analyze_text_sentiment(f'{symbol} Stock Analysis - Bullish Outlook'),
                'comments': [
                    {
                        'text': f'Great analysis on {symbol}!',
                        'author': 'viewer1',
                        'sentiment': self._analyze_text_sentiment(f'Great analysis on {symbol}!')
                    }
                ]
            }
        ]
        ...
        if mock_videos:
            ...
            logger.info(f"✅ YouTube sentiment: {len(mock_videos)} videos")
...
async def main():
    ...
    print("🚀 Enhanced Sentiment Integration Demo - FIXED VERSION")
    print("="*60)
    ...
    for source_name, source_data in sentiment_data['sources'].items():
        print(f"\n   {source_name.upper()}:")
        if 'sources' in source_data:
            for api_name, api_data in source_data['sources'].items():
                print(f"     {api_name}: {api_data.get('count', 0)} items")
